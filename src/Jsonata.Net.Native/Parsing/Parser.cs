...
internal sealed partial class Parser
{
    ...
    private readonly Dictionary<TokenType, Nud> nuds;
    private readonly Dictionary<TokenType, Led> leds;
    private readonly Dictionary<TokenType, int> bps;
    ...
    private Parser(string queryText)
    {
        ...
        this.nuds = new Dictionary<TokenType, Nud>() {
        ...
        this.leds = new Dictionary<TokenType, Led>() {
        ...
        this.bps = InitBindingPowers();
        ValidateBindingPowers(this.bps, this.leds);

        this.lexer = new Lexer(queryText);
    ...
    {
        this.token = this.lexer.next(allowRegex);
    ...
    private Node parseExpression(int rbp)
    {
        ...
        if (!this.nuds.TryGetValue(t.type, out Nud? nud))
        {
            throw new JsonataException("S0211", $"The symbol {Lexer.TokenTypeToString(t.type)} cannot be used as a unary operator");
        };
        Node lhs = nud(t);
        while (rbp < this.bps[this.token.type])
        {
            t = this.token;
            this.advance(true);

            if (!this.leds.TryGetValue(t.type, out Led? led))
            {
