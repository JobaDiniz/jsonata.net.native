...
internal sealed class Lexer
{
    ...
    private Token scanRegex(char delim)
    {
        ...
        RegexToken result = new RegexToken(this.queryText.Substring(this.StartPos, this.CurrentPos - this.StartPos), this.StartPos);
    ...
    private Token scanName()
    {
        ...
        else if (this.lookupKeyword(this.queryText.Substring(this.StartPos, this.CurrentPos - this.StartPos), out TokenType tt))
        {
    ...
    private Token scanEscapedName(char quoteChar)
    {
        while (true)
        {
            ...
            {
                throw new JsonataException("S0105", $"Quoted property name must be terminated with a backquote (`): " + this.queryText.Substring(this.StartPos));
            };
    ...
    private Token scanString(char quoteChar)
    {
        while (true)
        {
            ...
            if (chOrNull == null)
            {
                throw new JsonataException("S0101", "String literal must be terminated by a matching quote: " + this.queryText.Substring(this.StartPos));
            };
            if (chOrNull.Value == quoteChar)
            {
                break;
    ...
    {
        if (this.CurrentPos <= this.queryText.Length - 2)
        {
            if (this.queryText[this.CurrentPos] == '/'
                && this.queryText[this.CurrentPos + 1] == '*'
            )
            {
                int closeIndex = this.queryText.IndexOf("*/", startIndex: this.CurrentPos + 2);
                if (closeIndex < 0)
                {
    ...
    {
        if (this.CurrentPos < this.queryText.Length
            && this.queryText[this.CurrentPos] == rune)
        {
            nextRune();
            return true;
        }
    ...
    {
        if (this.CurrentPos < this.queryText.Length
            && (this.queryText[this.CurrentPos] == r1
                || this.queryText[this.CurrentPos] == r2
            )
        )
        {
            nextRune();
    ...
    {
        if (this.CurrentPos < this.queryText.Length
            && isValid(this.queryText[this.CurrentPos]))
        {
            nextRune();
            return true;
        }
    ...
    {
        Token result = new Token(type, this.queryText.Substring(this.StartPos, this.CurrentPos - this.StartPos), this.StartPos);
        this.StartPos = this.CurrentPos;
        return result;
    }
    ...
    private char? nextRune()
    {
        ...
        char r = this.queryText[this.CurrentPos];
        this.CurrentPos += 1;
        return r;
    }
