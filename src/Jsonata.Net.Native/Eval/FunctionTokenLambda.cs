...
internal sealed class FunctionTokenLambda : FunctionToken
{
    ...
    private sealed class ProcessedSignature
    {
        ...
        private readonly List<ProcessedParam> paramsList;
        private readonly Regex resultRegex;

        internal ProcessedSignature(LambdaNode.Signature signature)
        {
            (this.resultRegex, this.paramsList) = BuildRegexForParamsList(signature.args);
        }
        ...
        internal List<JToken> ValidateAndAlign(List<JToken> args, JToken? inputAsContext)
        {
            ...
            Match match = this.resultRegex.Match(suppliedSignature);
            if (!match.Success)
            {
            ...
            else
            {
                ...
                int argIndex = 0;
                for (int paramIndex = 0; paramIndex < this.paramsList.Count; ++paramIndex)
                {
                    ProcessedParam param = this.paramsList[paramIndex];
