...
public sealed class Tests
{
    ...
    private Dictionary<string, Jsonata.Net.Native.Json.JToken> datasets = new Dictionary<string, Jsonata.Net.Native.Json.JToken>();
    private readonly Dictionary<string, string> disabledTests = new Dictionary<string, string>() {
    ...
    private readonly Dictionary<string, string> suppressedTests = new Dictionary<string, string>() {
    ...
    public Tests()
    {
        ...
        foreach (string file in Directory.EnumerateFiles(datasetDirectory, "*.json"))
        {
            ...
            this.datasets.Add(Path.GetFileNameWithoutExtension(file), dataset);
        }
        Assert.NotEmpty(this.datasets);
        Console.WriteLine($"Loaded {this.datasets.Count} datasets");
    }
    ...
    public void Test(CaseInfo caseInfo)
    {
        ...
        {
            if (this.disabledTests.TryGetValue(caseInfo.testName!, out string? justification))
            {
        ...
        {
            if (!this.datasets.TryGetValue(caseInfo.dataset, out Jsonata.Net.Native.Json.JToken? datset))
            {
        ...
        try
        {
            ...
            if (this.suppressedTests.TryGetValue(caseInfo.testName!, out string? justification))
            {
